...
export const fileSystem = {
  ...
  // Documents
  async getDocuments(status: 'draft' | 'published'): Promise<DocumentMetadata[]> {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            try {
                ...
                const fileContent = await fs.readFile(path.join(fullPath, 'index.md'), 'utf-8');
  ...
  async findDocumentPath(id: string, status: 'draft' | 'published'): Promise<{ filePath: string, folderPath: string } | null> {
    const dir = path.join(DOCUMENTS_DIR, status);
    
    async function traverse(currentDir: string): Promise<{ filePath: string, folderPath: string } | null> {
        try {
            const entries = await fs.readdir(currentDir, { withFileTypes: true });
            for (const entry of entries) {
                const fullPath = path.join(currentDir, entry.name);
  ...
  async saveDocument(doc: Document): Promise<void> {
    ...
    const existingPublished = await this.findDocumentPath(metadata.id, 'published');
    
    const existing = existingDraft || existingPublished;
    const oldStatus = existingDraft ? 'draft' : (existingPublished ? 'published' : null);
  ...
  async deleteDocument(id: string): Promise<void> {
    const existing = await this.findDocumentPath(id, 'draft') || await this.findDocumentPath(id, 'published');
    if (existing) {
