...
<template>
  <div v-if="route.path !== '/'" class="fixed top-[53px] left-0 w-full z-40 bg-black bg-opacity-80 backdrop-blur border-b border-gray-800">
    <div class="container mx-auto px-4 py-2 flex items-center justify-between">
      <div class="flex items-center text-sm font-mono text-gray-400">
        <router-link to="/" class="hover:text-cyber-neon transition-colors flex items-center">
          <Home class="w-4 h-4 mr-1" />
        </router-link>
        
        <template v-for="(item, index) in breadcrumbs" :key="item.path">
          <ChevronRight v-if="index > 0" class="w-4 h-4 mx-2 text-gray-600" />
          <router-link 
            v-if="index > 0"
            :to="item.path"
            class="hover:text-cyber-neon transition-colors"
            :class="{ 'text-cyber-neon font-bold': index === breadcrumbs.length - 1 }"
          >
            {{ item.name }}
          </router-link>
        </template>
